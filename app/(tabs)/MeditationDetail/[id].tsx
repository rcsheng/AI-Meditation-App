// app/(tabs)/MeditationDetail/[id].tsx
...
const MeditationDetail: React.FC<MeditationDetailProps> = ({ navigation }) => {
  ...
  // This useEffect should handle both audio mode setup AND initial URL fetch
  // if you want them to happen only once on component mount.
  useEffect(() => {
  ...
  useEffect(() => {
    const fetchAudio = async () => {
      try {
        ...
        const audioPath = 'meditation-tracks/' + audioUrl + '.mp3';
  ...
  // Also add cleanup when the component unmounts
  // useEffect(() => {
  //   return () => {
  //     if (player) {
  //       console.log('Component unmounting, cleaning up audio player...');
  //       player.pause();
  //       player.remove();
  //       setPlayer(null);
  //       setIsPlaying(false);
  //     }
  //   };
  // }, [player]); // Add player to dependencies to ensure cleanup runs when player changes
  ...
  return (
    <View style={styles.container}>
      ...
      {/* Use MediaCard component to display the meditation details */}
      <View style={styles.mediaCardContainer}>
        <MediaCard item={{ id, title, author, length, audioUrl, imageUrl }} />
      </View>
...
const styles = StyleSheet.create({
  ...
  mediaCardContainer: {
    ...
    shadowOpacity: 0.1,
    shadowRadius: 4,
    height: height * 0.5, // Adjust height to 60% of the screen height
  },
  backButton: {
    marginBottom: 16,
  },
